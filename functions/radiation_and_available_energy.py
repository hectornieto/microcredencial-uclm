...
print("Gracias! liberías importadas")
print("Puedes continuar ahora con las siguientes tareas")
...
w_cab = w.FloatSlider(value=inv.MEAN_CAB,
                      min=inv.MIN_CAB,
                      max=inv.MAX_CAB,
                      step=1, description=r'Cab ($\mu$g/cm²)',
                      description_tooltip="Concetración de clorofila en la hoja",
                      **slide_kwargs)
...
w_soil = w.Dropdown(options=SOIL_TYPES,
                    ...
                    description_tooltip='Escoge un tipo de suelo',)
...
w_hb_ratio = w.FloatSlider(min=0.0, max=0.9, value=0.5, step=0.01,
                           ...
                           description_tooltip='Altura relativa del inicio de la copa',
                           **slide_kwargs)

w_wc = w.FloatSlider(min=0.1, max=3, value=1, step=0.01, description='Forma',
                     description_tooltip="Factor de forma del dosel (relación entre el ancho y el alto de copa)",
                     **slide_kwargs)
...
w_psi = w.FloatSlider(min=-90., max=90, value=0, description='Row azimuth (deg.)',
                      description_tooltip='Ángulo azimutal de las hileras',
                      **slide_kwargs)
...
w_sdn = w.FloatSlider(min=0., max=400, value=300,
                      description=r'S$\downarrow$ (W/m²)',
                      description_tooltip='Irradiancia solar diaria',
                      **slide_kwargs)
...
def plot_attenuation(length, tau):
    ...
    axs.set_xlabel('Camino óptico (unidades arbitrarias)')
    axs.set_ylabel('Transmisividad')
    axs.set_ylim((0, 1))
...
def plot_fipar(lai, leaf_angle):
    ...
    axs.set_xlabel('Ańgulo cenital solar (º)')
    axs.set_ylabel('Fracción de radiación interceptada')
    axs.set_ylim((0, 1.05))
    axs.set_xlim((0, 90))
    plt.tight_layout()
    plt.show()
...
def plot_net_radiation_rows(lai, leaf_angle, h_c, f_c, row_distance, row_direction,
    ...
    fig, axs = plt.subplots(figsize=FIGSIZE)
    # Plot the results
    axs.plot(HOURS, sn, 'black', label="Dosel en hileras, total")
    axs.plot(HOURS, sn_0, 'black', ls="--", label="Dosel homogéneo, total")
    axs.plot(HOURS, sn_v, 'green', label="Dosel en hileras, copa")
    axs.plot(HOURS, sn_v_0, 'green', ls="--",label="Dosel homogéneo, copa")
    axs.set_xlabel('Hora')
    if sdn_day != 1:
        axs.set_ylabel('Radiación absorbida (W m⁻²)')
    else:
        axs.set_ylabel('Fracción de radiación absorbida (--)')
...
def plot_net_solar_radiation(lai, leaf_angle, h_c, f_c, row_distance, row_direction,
    ...
    fig, axs = plt.subplots(figsize=FIGSIZE)
    # Plot the results
    axs.plot(HOURS, sn, 'black', label="Radiación neta")
    axs.plot(HOURS, sn_v, 'green', label="Radiación neta de la copa")
    axs.plot(HOURS, sn_s, 'orange', label="Radiación neta del suelo")
    axs.set_xlabel('Hora')
    axs.set_ylabel('Radiación solar (W m⁻²)')
    axs.set_xlim((TIME_START, TIME_END))
    axs.set_ylim((0, 1000))
    axs.legend()
    secax = axs.twinx()
    ...
    if f_c < 1:
        ...
        fig, axs = plt.subplots(figsize=FIGSIZE)
        # Plot the results
        axs.plot(HOURS, sn / sdn, 'black', label="Dosel en hilera, total")
        axs.plot(HOURS, sn_0, 'black', ls="--", label="Dosel homogéneo, total")
        axs.plot(HOURS, sn_v / sdn, 'green', label="Dosel en hilera, copa")
        axs.plot(HOURS, sn_v_0, 'green', ls="--", label="Dosel homogéneo, copa")
        axs.set_xlabel('Hora')
        axs.set_ylabel('Fracción de radiación absorbida (--)')
        axs.set_xlim((TIME_START, TIME_END))
...
def plot_longwave_radiation(t_air, hr, delta_t, emiss=0.98):
    ...
    plt.ylabel("Radiación neta de onda larga (W m⁻²)")
    plt.ylim((-200, 400))
    plt.tight_layout()
    plt.show()
    return ln
